"""Training script for TransNextConv model."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train TransNextConv Model",
    )

    # Data settings
    parser.add_argument(
        "--base_path", type=str, default=CONFIG["base_path"],
    ...
    parser.add_argument(
        "--train_split", type=float, default=CONFIG["train_split"],
        help="Train/val split ratio",
    )

    # Model settings
    parser.add_argument(
        "--n_channels", type=int, default=CONFIG["n_channels"],
    ...
    parser.add_argument(
        "--init_features", type=int, default=CONFIG["init_features"],
        help="Initial feature size",
    )
    parser.add_argument(
        "--spatial_size", type=int, nargs=2, default=CONFIG["spatial_size"],
    ...
    parser.add_argument(
        "--aux_loss_weight", type=float,
        default=CONFIG["aux_loss_weight"],
        help="Auxiliary loss weight",
    )
    parser.add_argument(
        "--load_balance_weight", type=float,
        default=CONFIG["load_balance_weight"],
    ...
    parser.add_argument(
        "--cache_rate", type=float, default=CONFIG["cache_rate"],
    ...
    # Logging
    parser.add_argument(
        "--use_wandb", action="store_true", default=CONFIG["use_wandb"],
        help="Enable W&B logging",
    )
    parser.add_argument(
        "--no_wandb", action="store_false", dest="use_wandb",
        help="Disable W&B logging",
    )
    parser.add_argument(
        "--wandb_project", type=str, default=CONFIG["wandb_project"],
        help="W&B project name",
    )
    parser.add_argument(
        "--wandb_api_key", type=str, default=CONFIG["wandb_api_key"],
        help="W&B API key",
    )
...
if __name__ == "__main__":
    ...
    best_checkpoint = torch.load(
        trainer.best_model_path, map_location=config["device"],
    )
    model.load_state_dict(best_checkpoint["model_state_dict"])
    print(
        f"Best model loaded from epoch {best_checkpoint['epoch']} "
        f"with Dice: {best_checkpoint['best_dice']:.4f}"
    )
