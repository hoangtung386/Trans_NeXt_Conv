"""Evaluation module for multi-organ segmentation model."""
...
class SegmentationEvaluator:
    """Comprehensive evaluation for multi-class segmentation models."""

    CLASS_NAMES = [
        "Background", "Liver", "Spleen", "Kidney_L", "Kidney_R", "Bowel",
    ]
    ...
    def _run_inference(self, images):
        """Run model inference and return output tensor."""
    ...
    def compute_metrics(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader):
                ...
                output = self._run_inference(images)
                ...
                for cls in range(1, self.num_classes):
                    ...
                    dice = (
                        (2 * intersection)
                        / (pred_cls.sum() + label_cls.sum() + 1e-8)
                    )
                    class_metrics[cls]["dice"].append(dice)

                    union = (
                        pred_cls.sum() + label_cls.sum() - intersection
                    )
                    ...
                    class_metrics[cls]["precision"].append(
                        tp / (tp + fp + 1e-8)
                    )
                    class_metrics[cls]["recall"].append(
                        tp / (tp + fn + 1e-8)
                    )
                    class_metrics[cls]["specificity"].append(
                        tn / (tn + fp + 1e-8)
                    )
    ...
    def plot_metrics(self, results, show=False):
        ...
        for idx, metric in enumerate(metrics_to_plot):
            ...
            axes[idx].set_title(
                f"{metric.upper()}", fontsize=14, fontweight="bold",
            )
        ...
        save_path = os.path.join(
            self.output_dir, "metrics_comparison_2d.png",
        )
    ...
    def visualize_predictions(self, num_samples=5):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                axes[0].set_title(
                    "CT Image", fontsize=14, fontweight="bold",
                )
                axes[0].axis("off")

                axes[1].imshow(img_np, cmap="gray")
                axes[1].imshow(
                    label_np, cmap=cmap, alpha=0.5, vmin=0, vmax=5,
                )
                axes[1].set_title(
                ...
                axes[2].imshow(
                    pred_np, cmap=cmap, alpha=0.5, vmin=0, vmax=5,
                )
                axes[2].set_title(
                ...
                save_path = os.path.join(
                    self.output_dir,
                    f"overlay_2d_sample_{samples_shown + 1}.png",
                )
                plt.savefig(save_path, dpi=150, bbox_inches="tight")
                plt.close()
        ...
        print(
            f"Overlay visualizations saved to "
            f"{self.output_dir}/overlay_2d_sample_*.png"
        )
    ...
    def plot_per_class_comparison(self, num_samples=3):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                fig, axes = plt.subplots(
                    2, self.num_classes, figsize=(20, 8),
                )

                for cls_idx in range(self.num_classes):
                    gt_mask = (label_np == cls_idx).astype(float)
                    axes[0, cls_idx].imshow(img_np, cmap="gray")
                    axes[0, cls_idx].imshow(
                        gt_mask, cmap="Reds", alpha=0.5, vmin=0, vmax=1,
                    )
                    ...
                    axes[1, cls_idx].imshow(
                        pred_mask, cmap="Blues", alpha=0.5, vmin=0, vmax=1,
                    )
                ...
                save_path = os.path.join(
                    self.output_dir,
                    f"per_class_comparison_{samples_shown + 1}.png",
                )
                plt.savefig(save_path, dpi=150, bbox_inches="tight")
                plt.close()
        ...
        print(
            f"Per-class comparison saved to "
            f"{self.output_dir}/per_class_comparison_*.png"
        )
    ...
    def plot_confusion_analysis(self):
        ...
        cm = confusion_matrix(
            all_labels, all_preds, labels=range(self.num_classes),
        )
        ...
        sns.heatmap(
            cm_normalized, annot=True, fmt=".3f", cmap="Greens",
            xticklabels=self.class_names,
            yticklabels=self.class_names,
        ...
        plt.tight_layout()
        save_path = os.path.join(
            self.output_dir, "confusion_matrix_2d.png",
        )
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def create_summary_report(self, results):
        ...
        report_path = os.path.join(
            self.output_dir, "evaluation_report_2d.txt",
        )
        ...
        with open(report_path, "w") as f:
            ...
            for metric in [
                ...
                avg = np.mean([
                    results[cls][metric] for cls in results
                ])
