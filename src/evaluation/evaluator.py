...
class SegmentationEvaluator:
    def __init__(self, model, val_loader, device, num_classes=6, output_dir='Trans_next_Conv/images/evaluation_results'):
        self.model = model
    ...
    def visualize_predictions(self, num_samples=5):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(self.output_dir, f'overlay_2d_sample_{samples_shown+1}.png'),
                           dpi=150, bbox_inches='tight')
                plt.show()

                samples_shown += 1

        print(f"Overlay visualizations saved to {self.output_dir}/overlay_2d_sample_*.png")
    ...
    def plot_per_class_comparison(self, num_samples=3):
        ...
        with torch.no_grad():
            for batch in self.val_loader:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(self.output_dir, f'per_class_comparison_{samples_shown+1}.png'),
                           dpi=150, bbox_inches='tight')
                plt.show()
                
                samples_shown += 1
        
        print(f"Per-class comparison saved to {self.output_dir}/per_class_comparison_*.png")
    ...
    def plot_confusion_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'confusion_matrix_2d.png'), dpi=150, bbox_inches='tight')
        plt.show()
        print(f"Confusion matrix saved to {self.output_dir}/confusion_matrix_2d.png")

    def create_summary_report(self, results):
        """Create text summary report"""
        report_path = os.path.join(self.output_dir, 'evaluation_report_2d.txt')
