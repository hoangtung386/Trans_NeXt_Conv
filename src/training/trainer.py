"""Trainer module for medical image segmentation."""
...
class Trainer:
    """Training pipeline with AMP, gradient accumulation, and checkpointing."""
    ...
    def _forward_and_loss(self, images, masks):
        ...
        Returns:
            Tuple of (output, loss, loss_dict).
        """
        output, all_aux, aux1, aux2, aux3, _gates = self.model(images)
        ...
        loss, loss_dict = self.criterion(
            output, masks, aux_losses=aux_losses,
        )
    ...
    def train_epoch(self, epoch):
        ...
        self.model.train()
        total_losses = {
            "total": 0, "dice": 0, "focal": 0, "tversky": 0, "aux": 0,
        }
        ...
        for step, batch in enumerate(pbar):
            ...
            masks = self._prepare_masks(batch["seg"].to(self.device))

            if self.use_amp:
                with autocast("cuda"):
                    output, loss, loss_dict = self._forward_and_loss(
                        images, masks,
                    )
    ...
    def train(self, num_epochs=None):
        ...
        print(f"\n{'=' * 60}")
        print(
            f"Starting training from epoch {self.start_epoch} to {num_epochs}"
        )
