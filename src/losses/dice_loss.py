"""Dice Loss and Generalized Dice Loss for segmentation tasks."""
...
class DiceLoss(nn.Module):
    ...
    def forward(
        ...
        if self.ignore_index is not None:
            mask = target[
                :, self.ignore_index:self.ignore_index + 1, :, :
            ].bool()
            output = output * (~mask)
            target = target * (~mask)

        dice_score = soft_dice_score(
            output, target, self.smooth, self.eps, dims=(0, 2, 3),
        )
...
class GeneralizedDiceLoss(nn.Module):
    ...
    def forward(
        ...
        gdl = 1.0 - (
            (2.0 * weighted_intersection + self.smooth)
            / (weighted_cardinality + self.smooth)
        )
